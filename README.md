...
`market data → SCALP-Ω data/feature engine → TITAN deterministic decision engine → trading decision`

SCALP-Ω is the local decision authority. The final `LONG`, `SHORT`, or `NO_TRADE` decision is generated by deterministic TITAN/confluence/risk gates from the supplied market evidence.
...
SCALP-Ω makes trading decisions locally from its deterministic TITAN/confluence/risk engines. No external model API or model key is required.
