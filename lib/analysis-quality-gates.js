...
function freshnessGate({ candlesByTf, now }) {
  ...
  for (const [tf, maxAgeMs] of Object.entries(REQUIRED_TIMEFRAMES)) {
    ...
    const rawAgeMs = finite(latestTs) ? Number(now) - latestTs : null;
    const ageMs = finite(rawAgeMs) ? Math.max(0, rawAgeMs) : null;
    const future = finite(rawAgeMs) && rawAgeMs < -5000;
    const pass = finite(ageMs) && !future && ageMs <= maxAgeMs;
    byTimeframe[tf] = { pass, latestTs, ageMs, maxAgeMs, future };
    if (!pass) reasons.push(`STALE_OR_MISSING_${tf}`);
    if (future) reasons.push(`FUTURE_TIMESTAMP_${tf}`);
  }
...
function sourceConsensus({ market, externalIntelligence, now }) {
  ...
  for (const name of ['binance', 'bybit', 'deribit']) {
    ...
    if (provider?.available !== false && finite(provider?.price)) {
      rows.push({ name: name.toUpperCase(), price: Number(provider.price), fundingRate: provider.fundingRate, openInterest: provider.openInterest, timestamp: providerTimestamp(provider, null) });
    }
  }
  const reasons = [];
  ...
  const untimestampedSources = rows.filter((row) => !finite(row.timestamp)).map((row) => row.name);
  if (untimestampedSources.length) reasons.push(`MISSING_SOURCE_TIMESTAMP_${untimestampedSources.join('_')}`);
  const staleSources = rows.filter((row) => finite(row.timestamp) && Number(now) - row.timestamp > 2 * 60 * 1000).map((row) => row.name);
  ...
  return { pass: reasons.length === 0, reasons, providers: rows, spreadBps, fundingSpread, untimestampedSources, staleSources };
}
...
export function evaluateAnalysisQualityGates({
  ...
  const groupPasses = Object.values(groups).map((group) => group.pass === true);
  const score = Number(((groupPasses.filter(Boolean).length / Math.max(1, groupPasses.length)) * 10).toFixed(2));
  const pass = strict ? reasons.length === 0 : freshness.pass && sources.pass;
  const grade = pass ? '10/10' : score >= 7.5 ? 'PARTIAL_7.5+' : score >= 5 ? 'PARTIAL_5+' : 'BLOCKED';
  return {
    enabled: true,
    strict,
    pass,
