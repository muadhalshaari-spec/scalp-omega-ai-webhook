...
async function getPublicMarketData({ signal } = {}) {
  const publicEntries = [
    ...
    { label:'futures.takerLongShort', base:USD_M_BASE, path:'/futures/data/takerlongshortRatio', params:{symbol:SYMBOL,period:'15m',limit:500} },
    { label:'futures.takerFlowRatio', base:USD_M_BASE, path:'/futures/data/takerlongshortRatio', params:{symbol:SYMBOL,period:'15m',limit:500} }
  ];
  ...
  for (const period of SENTIMENT_PERIODS) {
    publicEntries.push(
      ...
      { label:`futures.takerFlowRatio.${period}`, base:USD_M_BASE, path:'/futures/data/takerlongshortRatio', params:{symbol:SYMBOL,period,limit:500} },
  ...
  const futures = {
    ...
    histories: {
      ...
      takerLongShort: Object.fromEntries(SENTIMENT_PERIODS.map((p) => [p, valueOf(futuresResults,`futures.takerLongShort.${p}`) || []])),
      takerBuySell: Object.fromEntries(SENTIMENT_PERIODS.map((p) => [p, valueOf(futuresResults,`futures.takerFlowRatio.${p}`) || []])),
