...
export function buildInstitutionalAnalysis({
  ...
  for (const tf of TF_ORDER) {
    ...
    regimes[tf] = classifyRegime(clean[tf]);
    structures[tf] = buildMarketStructure(clean[tf]);
  }

  const c15 = clean['15m'];
  const structure15 = structures['15m'];
  const structure1h = structures['1H'];
  const liquidity = buildLiquidityMap(c15, structure15);
  const zones = zoneAt(buildZones(c15), c15.at(-1)?.close || 0);
  ...
  const setupCandidates = detectSetups({
    regime: regimes['15m'],
  ...
  const htfScores = [
    regimeScore(regimes['1H']),
    regimeScore(regimes['4H']),
    regimeScore(regimes['1D'])
  ];
  const executionScores = [
    regimeScore(regimes['15m']),
    regimeScore(regimes['5m'])
  ];
  const structuralScores = [
    structureScore(structures['15m']),
    structureScore(structures['1H'])
  ];
