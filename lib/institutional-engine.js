...
import { buildExecutionPlan } from './execution-plan-engine.js';
import { buildAnalogFeature, findHistoricalAnalogs, analogConfidence } from './historical-analog-engine.js';
...
import { runTitanPipeline } from './titan/index.js';
import { evaluateAnalysisQualityGates } from './analysis-quality-gates.js';
...
export function buildInstitutionalAnalysis({
  ...
  const derivatives = analyzeDerivatives({
    ...
    takerVolumeHistory: market.takerVolumeHistory || [],
    longShortHistory: market.longShortHistory || []
  });
  ...
  const setupCandidates = c15.length >= 220 && structure15 && regimes['15m'] ? detectSetups({
    ...
    executionRegime: regimes['5m'],
    executionStructure: structures['5m'],
    mode
  }) : [];
  ...
  const derivativesBias =
    derivatives.pressure === 'LONG_CROWDING' ? 0.42 :
    derivatives.pressure === 'SHORT_CROWDING' ? 0.58 :
    derivatives.pressure === 'LONG_UNWIND' ? 0.58 :
    derivatives.pressure === 'SHORT_UNWIND' ? 0.42 :
    0.5;
  ...
  const qualityFactor = mean([
    ...
    derivatives.pressure === 'LONG_CROWDING' || derivatives.pressure === 'SHORT_CROWDING' ? 0.55 : 0.75
  ]) ?? 0.5;

  const session = sessionContext(timestamp);
  const analogFeature = buildAnalogFeature({
    direction,
    setup,
    regime: regimes['15m'],
    features: feature['15m'],
  ...
  const analogs = findHistoricalAnalogs(analogFeature, historicalAnalogs);
  const analogWeight = analogConfidence(analogs);
  ...
  const eventRisk = buildEventRisk({
    timestamp,
    funding: {
      fundingTime: derivatives.funding.fundingTime,
      nextFundingTime: derivatives.funding.nextFundingTime
    },
    externalEvents
  });
  const reasons = [];
  const requestedDirection = direction === 'NONE' ? probability.direction : direction;
  const qualityGates = evaluateAnalysisQualityGates({
    candlesByTf: clean,
    features: feature,
    market,
  ...
  if (direction === 'NO_TRADE' || direction === 'NONE') reasons.push('NO_ACTIONABLE_SETUP');
  ...
  if (!qualityGates.pass) reasons.push('ANALYSIS_QUALITY_GATE');
  reasons.push(...qualityGates.reasons);

  const externalAgreement = externalIntelligence?.crossExchange?.agreement;
  const externalFundingCrowding = externalIntelligence?.featureSignals?.fundingCrowding;
  const externalFundingDirection = externalIntelligence?.featureSignals?.fundingDirection;
  if (externalAgreement === 'DIVERGENT') reasons.push('EXTERNAL_PRICE_DIVERGENCE');
  if (externalFundingCrowding != null && externalFundingCrowding > 0.9) reasons.push('EXTREME_EXTERNAL_FUNDING_CROWDING');
  if (externalFundingDirection === 1 && requestedDirection === 'SHORT') reasons.push('EXTERNAL_FUNDING_CONFLICT');
  if (externalFundingDirection === -1 && requestedDirection === 'LONG') reasons.push('EXTERNAL_FUNDING_CONFLICT');
  ...
  const titanBaseDecision = decision;

  const titan = runTitanPipeline({
    timestamp,
    decisionTimestamp: timestamp,
    sourceTimestamp: timestamp,
    mode,
    decision: titanBaseDecision,
    direction: titanBaseDecision,
    candlesByTf: clean,
    candles: c15,
    candles15m: c15,
    candles5m: clean['5m'],
    ...
    market: {
      ...market,
      price: realtime.price || market.price,
      orderBook: realtime.orderBook || market.orderBook,
      trades: realtime.trades || market.trades || []
    },
    realtime,
    ...
    strategyReturns: [],
    statusEvidence: {},
    moduleSelfTestStatus: {}
  });

  const titanKill = titan?.outputs?.['TITAN-39']?.result?.active === true;
  const titanData = titan?.outputs?.['TITAN-17']?.result?.ready === false;
  const titanEvent = titan?.outputs?.['TITAN-16']?.result?.tradeAllowed === false;
  const titanEntry = titan?.outputs?.['TITAN-03']?.result?.direction || titan?.outputs?.['TITAN-03']?.state?.direction || 'NO_TRADE';
  const titanRiskBlocked = titan?.outputs?.['TITAN-12']?.result?.riskAllowed === false;

  const titanDecision = titanKill || titanData || titanEvent || titanRiskBlocked
    ? 'NO_TRADE'
    : (titanEntry === titanBaseDecision ? titanBaseDecision : 'NO_TRADE');

  const finalDecision = titanDecision;
  const finalRisk = finalDecision === 'NO_TRADE' ? null : risk;
  const chatgptOnly = String(mode).toLowerCase() === 'live';

  const executionPlan = chatgptOnly ? null : buildExecutionPlan({
    decision: finalDecision,
    setup,
    risk: finalRisk,
    probability,
    marketPrice: realtime.price || market.price,
    liquidity,
    zones,
    regime: regimes['15m'],
    timestamp
  ...
  return {
    ...
    decision: chatgptOnly ? null : finalDecision,
    deterministicDecision: chatgptOnly ? null : titanBaseDecision,
    titanDecision: chatgptOnly ? null : titanDecision,
    decisionAuthority: chatgptOnly ? 'CHATGPT_ONLY' : 'SYSTEM_ANALYSIS_MODE',
    decisionPolicy: chatgptOnly ? 'SYSTEM_PROVIDES_DATA_ONLY' : 'SYSTEM_ANALYSIS_MODE',
    titan,
    probability,
    setup,
    setupCandidates,
    ...
    eventRisk,
    qualityGates,
    risk: chatgptOnly ? null : finalRisk,
    journal,
    executionPlan,
    externalIntelligence,
    meta: {
    ...
    componentScores: {
      ...
      derivatives: derivativesBias,
      sequenceFactor,
      qualityFactor
    },
    thresholds: thresholds || { long: 0.72, short: 0.72, minEdge: 0.12 },
    noTradeReasons: [...reasons, ...(finalDecision === 'NO_TRADE' && titanBaseDecision !== 'NO_TRADE' ? ['TITAN_55_GATE'] : [])],
    dataQuality: data,
    layerStatus: {
      data: true,
      structure: true,
      ...
      probability: true,
      analysisQualityGates: qualityGates.pass,
      historicalAnalogs: true,
      titan55: titan?.summary
    },
    researchMode: mode,
    counterfactual: {
      evidenceDirection,
      candidateDirection,
      decisionDirectionGap: candidateDirection !== 'NONE' && evidenceDirection !== 'NO_TRADE'
        ? candidateDirection === evidenceDirection ? 'ALIGNED' : 'CONFLICTED'
        : 'NEUTRAL'
    }
  };
}
