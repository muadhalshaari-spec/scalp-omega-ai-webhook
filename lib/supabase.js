...
export async function insertIndicatorFeatures(rows){const baseUrl=supabaseBaseUrl();if(!baseUrl||!key)return{configured:false,persisted:false,rows:0};if(!Array.isArray(rows)||!rows.length)return{configured:true,persisted:true,rows:0};let written=0;for(let i=0;i<rows.length;i+=500){const batch=rows.slice(i,i+500);const response=await fetch(baseUrl+'/rest/v1/indicator_features?on_conflict=indicator_id,instrument,timeframe,event_ts',{method:'POST',headers:authHeaders({'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'}),body:JSON.stringify(batch),cache:'no-store'});if(!response.ok){const t=await response.text();throw new Error('Supabase indicator_features HTTP '+response.status+': '+t.slice(0,500));}written+=batch.length}return{configured:true,persisted:true,rows:written,status:'PERSISTED'}}
