...
async function syncOne(state, signal) {
  ...
  const before = num(state.cursor_ms);
  ...
  try {
    ...
    if (!rows.length || batch.nextCursorMs == null || batch.nextCursorMs === before) {
      await updateSyncState({ source: state.source, instrument: state.instrument, timeframe: state.timeframe, status: Number(state.stored_rows || 0) >= target ? 'COMPLETE' : 'SOURCE_EXHAUSTED', last_batch_rows: rows.length, last_error: rows.length ? null : 'SOURCE_RETURNED_NO_ROWS', cursor_ms: batch.nextCursorMs ?? before, completed_at: Number(state.stored_rows || 0) >= target ? new Date().toISOString() : null });
      return { ...state, status: 'SOURCE_EXHAUSTED', batchRows: rows.length };
    }
