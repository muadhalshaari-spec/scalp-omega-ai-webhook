...
export async function redisSetIfAbsent(key, value, ttlSeconds = 30, { signal } = {}) {
  const r = await command(['SET', key, String(value), 'EX', String(Math.max(1, ttlSeconds)), 'NX'], { signal });
  return { ...r, acquired: r.configured && r.result === 'OK' };
}
