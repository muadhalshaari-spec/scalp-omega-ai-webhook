...
function enrichFundingSchedule(fundingHistory, currentFunding = {}) {
  const sorted = sortUnique(fundingHistory);
  return sorted.map((row, i) => ({
    ...row,
    nextFundingTime: row.nextFundingTime ??
      sorted[i + 1]?.ts ??
      toNumber(currentFunding.nextFundingTime, null)
  }));
}
...
export async function fetchDerivativeData({
  ...
  const currentFundingPromise = fetchJson(
    `${OKX_BASE}/api/v5/public/funding-rate?${query({ instId })}`,
    { signal }
  ).catch(() => ({ data: [] }));

  const currentOiPromise = fetchJson(
  ...
  const fundingHistory = enrichFundingSchedule(fundingHistoryRaw, fundingCurrent);
