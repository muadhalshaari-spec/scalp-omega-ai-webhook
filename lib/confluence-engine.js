...
function dominantDirectionHint(result) {
  return result.longScore > result.shortScore ? 'LONG' : result.shortScore > result.longScore ? 'SHORT' : 'NONE';
}
...
export function buildConfluence({ features, contexts, market }) {
  ...
  const ext = market?.externalIntelligence;
  const extAgreement = ext?.crossExchange?.agreement;
  const extPriceSignal = ext?.featureSignals?.priceAgreement;
  result.evidence.push({ name: 'Cross-exchange price agreement', weight: 0, direction: 'CONTEXT', detail: `agreement=${extAgreement || 'UNKNOWN'}, divergencePct=${ext?.crossExchange?.priceDivergencePct ?? null}, sources=${ext?.crossExchange?.priceSources ?? 0}` });
  if (extAgreement === 'DIVERGENT') result.noTradeReasons.push('Cross-exchange price divergence is elevated');
  const fundingDirection = ext?.featureSignals?.fundingDirection;
  const fundingCrowding = ext?.featureSignals?.fundingCrowding;
  result.evidence.push({ name: 'Cross-exchange funding', weight: 0, direction: 'CONTEXT', detail: `mean=${ext?.crossExchange?.fundingMean ?? null}, dispersion=${ext?.crossExchange?.fundingDispersion ?? null}, crowding=${fundingCrowding ?? null}` });
  result.evidence.push({ name: 'Deribit options', weight: 0, direction: 'CONTEXT', detail: `putCallOI=${ext?.providers?.deribit?.options?.putCallOI ?? null}, IV=${ext?.providers?.deribit?.options?.weightedIV ?? null}` });
  if (extPriceSignal != null && extPriceSignal < 0.6) result.noTradeReasons.push('External price-source agreement is weak');
  if (fundingCrowding != null && fundingCrowding > 0.9) result.conflicts.push('Extreme cross-exchange funding crowding');
  if (fundingDirection === 1 && dominantDirectionHint(result) === 'SHORT') result.conflicts.push('Positive funding crowding conflicts with short bias');
  if (fundingDirection === -1 && dominantDirectionHint(result) === 'LONG') result.conflicts.push('Negative funding crowding conflicts with long bias');
