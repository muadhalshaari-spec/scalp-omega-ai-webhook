...
export function buildConfluence({ features, contexts, market, glassnode = null, bybit = null }) {
  ...
  const bybitBook = bybit?.orderBook?.imbalance;
  const bybitDir = bybit?.available && Number.isFinite(bybitBook)
    ? (bybitBook > 0.08 ? 'BULLISH' : bybitBook < -0.08 ? 'BEARISH' : 'NEUTRAL')
    : 'NEUTRAL';
  addEvidence(result, 'Bybit market confirmation', 7.5, bybitDir,
    `bookImbalance=${bybitBook ?? null}; last=${bybit?.ticker?.lastPrice ?? null}; mark=${bybit?.ticker?.markPrice ?? null}`);
  if (bybit?.configured && !bybit?.available) result.noTradeReasons.push('Bybit market confirmation unavailable');
  if (glassnode?.configured && !glassnode?.quality?.available) {
