    ...
    if (macd[i] != null) {
      ...
      result.set(closed[i].time, {
        ...
        indicators: {
          ...
          vwap: (() => { const from=Math.max(0,i-199); let pv=0,v=0; for(let k=from;k<=i;k++){ const x=closed[k]; const t=(x.high+x.low+x.close)/3; pv+=t*x.volume; v+=x.volume; } return v?pv/v:null; })(),
...
function scoreAt(signalTime, series, candlesByTf) {
  const get = tf => {
  ...
  const contextAt = (tf) => {
    const all = candlesByTf[tf]?.filter(c => c.confirmed && c.time <= signalTime) || [];
    return all.length >= 20 ? buildMarketContext(all.slice(-300), null) : null;
  };
  const s15 = contextAt('15m'), s1h = contextAt('1H');
  const structureDir = (() => {
    const a=s15?.structure?.structureEvent, b=s1h?.structure?.structureEvent;
    if (['BOS_BULLISH','BREAK_HIGH'].includes(a) || ['BOS_BULLISH','BREAK_HIGH'].includes(b)) return 'BULLISH';
    if (['BOS_BEARISH','BREAK_LOW'].includes(a) || ['BOS_BEARISH','BREAK_LOW'].includes(b)) return 'BEARISH';
    return 'NEUTRAL';
  })();
  if (structureDir === 'BULLISH') longScore += 20;
  if (structureDir === 'BEARISH') shortScore += 20;

  const c5 = (candlesByTf['5m']||[]).filter(c=>c.confirmed&&c.time<=signalTime).slice(-300);
  const s5 = c5.length>=20 ? buildMarketContext(c5,null) : null;
  const liquidityDir = (s15?.liquidity?.sweep==='SELL_SIDE_SWEPT'||s5?.liquidity?.sweep==='SELL_SIDE_SWEPT')
    ? 'BULLISH'
    : (s15?.liquidity?.sweep==='BUY_SIDE_SWEPT'||s5?.liquidity?.sweep==='BUY_SIDE_SWEPT')
      ? 'BEARISH' : 'NEUTRAL';
  if (liquidityDir === 'BULLISH') longScore += 15;
  if (liquidityDir === 'BEARISH') shortScore += 15;
  ...
  if (f15) {
    ...
    const vw = f15.indicators.vwap;
    if (vw != null && f15.lastCandle.close > vw) longScore += 10;
    else if (vw != null && f15.lastCandle.close < vw) shortScore += 10;
    if (v >= 1.25) {
...
export function runBacktest({ candlesByTf, horizonBars = 48, rr = 2 }) {
  ...
  for (let i = start; i < base.length - horizonBars; i++) {
    const candle = base[i];
    const decision = scoreAt(candle.time, series, candlesByTf);
