const DERIBIT_BASE_URL = (process.env.DERIBIT_BASE_URL || (process.env.DERIBIT_ENV || 'testnet').toLowerCase() === 'production'
  ? 'https://www.deribit.com/api/v2'
  : 'https://test.deribit.com/api/v2');

function credentials() {
  return {
    clientId: (process.env.DERIBIT_CLIENT_ID || '').trim(),
    clientSecret: (process.env.DERIBIT_CLIENT_SECRET || '').trim()
  };
}
...
export function deribitEnvironment() {
  return DERIBIT_BASE_URL.includes('test.deribit.com') ? 'testnet' : 'production';
}
...
async function rpc(method, params = {}) {
  ...
  if (!response.ok || data?.error) throw new Error(`Deribit HTTP ${response.status}: ${data?.error?.message || body.slice(0, 300)}`);
...
async function privateRpc(method, params = {}) {
  ...
  const tokenResponse = await fetch(tokenUrl, { method: 'GET', headers: { Accept: 'application/json' }, cache: 'no-store' });
  const tokenBody = await tokenResponse.text();
  let tokenData; try { tokenData = JSON.parse(tokenBody); } catch {}
  ...
  const response = await fetch(DERIBIT_BASE_URL + '/private/' + method, {
    method: 'POST', headers: { Accept: 'application/json', Authorization: `Bearer ${tokenData.result.access_token}`, 'Content-Type': 'application/json' },
  ...
  if (!response.ok || data?.error) throw new Error(`Deribit private HTTP ${response.status}: ${data?.error?.message || body.slice(0, 300)}`);
  return data?.result ?? null;
}
...
export async function getDeribitMarket(instrument = 'ETH-PERPETUAL') {
  ...
  return { instrument, ticker, orderBook: book, summary: Array.isArray(summary) ? summary[0] || null : summary };
}

export async function getDeribitAccount(currency = 'ETH') { return privateRpc('get_account_summary', { currency, extended: true }); }

export async function testDeribitCredentials() {
  const account = await getDeribitAccount('ETH');
  return { authenticated: true, environment: deribitEnvironment(), currency: account?.currency || 'ETH', accountType: account?.type || null, equity: account?.equity ?? null, availableFunds: account?.available_funds ?? null };
}
