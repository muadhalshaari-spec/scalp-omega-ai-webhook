...
export async function getExecutionAudit(decisionId){
  ...
  const q=new URLSearchParams({select:'*',decision_id:'eq.'+encodeURIComponent(String(decisionId)),limit:'1'});
...
export async function updateExecutionAudit(decisionId,patch){
  const q=new URLSearchParams({decision_id:'eq.'+encodeURIComponent(String(decisionId))});
