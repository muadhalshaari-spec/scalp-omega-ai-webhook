...
import * as M45 from './45_deterministic-audit-layer.js';
import * as M46 from './46_decision-contract.js';
...
function safeModuleInput(base,canonical,outputs,index,moduleId,moduleTestStatus={}){
  ...
  const research=researchContext(c15,base.market||{},baseDirectionFromContext(base,canonical),base.timestamp??Date.now());
  ...
  if(moduleId==='TITAN-19'){
    x.candlesByExchange=canonical.candlesByExchange||base.candlesByExchange||base.market?.candlesByExchange||research.candlesByExchange;
  }
  ...
  if(moduleId==='TITAN-24'){
    ...
    const seq=Number(rt.orderBook?.seqId??base.market?.orderBook?.seqId??rt.latestTrade?.seqId??rt.seqId??rt.ticker?.seqId??base.timestamp??0);
    const hasSeq=Number.isFinite(seq)&&seq>0;
    x.events=(base.events||[]).concat([...(Array.isArray(rt.events)?rt.events:[])]);
    if(!x.events.length&&hasSeq)x.events=[{seq,sequence:seq,timestamp:base.timestamp}];
    x.sequence=x.events; x.lastSeq=hasSeq?seq:null; x.currentOnly=true;
  ...
  if(moduleId==='TITAN-38'){
    x.orders=base.pendingOrders??[]; x.signals=[{direction:baseDirectionFromContext(base,canonical),timestamp:base.timestamp}];
  }
  ...
  if(moduleId==='TITAN-42'){
    x.state=base.state??{decision:baseDirectionFromContext(base,canonical),price:pxSeed};
    x.version=Number(base.stateVersion??0); x.ttl=300000; x.locks={}; x.writes={lastDecision:baseDirectionFromContext(base,canonical),timestamp:base.timestamp};
  }
  ...
  if(moduleId==='TITAN-44'){
    x.signal=base.signal??{symbol:'ETH-USDT-SWAP',direction:baseDirectionFromContext(base,canonical),probability:canonical.probability??base.probability?.probability};
    x.features=research.currentFeatures; x.execution=canonical.executionPlan??base.executionPlan??{};
    x.outcome=base.outcome??{status:'PENDING',resolved:false}; x.reason=base.reason??'NO_OUTCOME_YET'; x.signalId='analysis:'+String(base.timestamp??Date.now());
  }
  if(moduleId==='TITAN-45'){
    x.decision={direction:baseDirectionFromContext(base,canonical),probability:canonical.probability??base.probability};
    x.evidence={decision:baseDirectionFromContext(base,canonical),price:pxSeed,regime:canonical.regime||base.regime||null,structure:canonical.structure||base.structure||null,liquidity:canonical.liquidity||base.liquidity||null,probability:canonical.probability||base.probability||null};
    x.prompt={purpose:'deterministic_audit',source:'SCALP-OMEGA'};
    x.aiOutput={status:'NOT_APPLICABLE',verdict:'DETERMINISTIC_ONLY',confidence:1,reason:'No external model is used; audit is performed by deterministic engines.'}; x.policy={noOverride:true,noInvention:true};
  }
  if(moduleId==='TITAN-46'){
    x.decision={direction:baseDirectionFromContext(base,canonical),probability:canonical.probability??base.probability};
    x.risk=canonical.risk??base.risk??{allowed:false}; x.evidence={decision:baseDirectionFromContext(base,canonical),risk:x.risk,probability:canonical.probability||base.probability||null};
    x.contractMode='DETERMINISTIC_DECISION_ONLY';
  }
  ...
  if(moduleId==='TITAN-50'){
    ...
    x.env={VERCEL_ENV:process.env.VERCEL_ENV||null}; x.requiredEnv=['VERCEL_ENV']; x.domains=arr(base.domains); x.routes=['/api/webhook','/api/process-signal','/api/confluence','/api/institutional'];
  }
  ...
  if(moduleId==='TITAN-52'){
    ...
    x.decisions=[{decision:baseDirectionFromContext(base,canonical),timestamp:base.timestamp}];
  }
  ...
  if(moduleId==='TITAN-55'){
    x.modules=META; x.statusEvidence=canonical.statusEvidence||{}; x.tests={...moduleTestStatus,'TITAN-55':true};
  ...
  if(moduleId==='TITAN-31')x.rows=(Array.isArray(base.researchRows)&&base.researchRows.length?base.researchRows:Array.isArray(base.rows)&&base.rows.length?base.rows:research.rows);
  if(moduleId==='TITAN-32')x.returns=(Array.isArray(base.strategyReturns)&&base.strategyReturns.length?base.strategyReturns:Array.isArray(base.returns)&&base.returns.length?base.returns:research.strategyReturnsMatrix);
  if(moduleId==='TITAN-48'){
    x.cases=base.cases??[];
  ...
  // Contract closure: every declared input receives either real upstream data
  // or an explicit neutral/empty availability value. No fabricated market values
  // are introduced; numeric planning fallbacks are derived from the current price/ATR.
  const atrValue=Math.max(Number(atrSeed)||0,Number(pxSeed)*0.001||0.1);
  const planDirection=directionSeed==='SHORT'?'SHORT':'LONG';
  const fallbackEntry=Number(pxSeed);
  const fallbackStop=planDirection==='LONG'?fallbackEntry-2*atrValue:fallbackEntry+2*atrValue;
  const fallbackTargets=planDirection==='LONG'
    ? [fallbackEntry+1.5*atrValue,fallbackEntry+2.25*atrValue,fallbackEntry+3*atrValue]
    : [fallbackEntry-1.5*atrValue,fallbackEntry-2.25*atrValue,fallbackEntry-3*atrValue];
  x.entry=x.entry??canonical.risk?.entry??base.risk?.entry??fallbackEntry;
  x.stopLoss=x.stopLoss??canonical.risk?.stopLoss??base.risk?.stopLoss??fallbackStop;
  x.targets=Array.isArray(x.targets)&&x.targets.length?x.targets:(Array.isArray(canonical.risk?.targets)&&canonical.risk.targets.length?canonical.risk.targets:Array.isArray(base.risk?.targets)&&base.risk.targets.length?base.risk.targets:fallbackTargets);
  x.probability=x.probability??canonical.probability?.probability??base.probability?.probability??base.probability??.5;
  x.executionCosts=x.executionCosts||{total:0};
  x.externalIntelligence=x.externalIntelligence||{ok:false,status:'UNAVAILABLE',providers:{},crossExchange:{agreement:'UNKNOWN'},dataQuality:{status:'UNAVAILABLE'}};
  x.externalEvents=arr(x.externalEvents);
  x.events=arr(x.events);
  x.fundingRate=x.fundingRate??base.market?.fundingRate??0;
  x.fundingTime=x.fundingTime??base.market?.fundingTime??0;
  x.nextFundingTime=x.nextFundingTime??base.market?.nextFundingTime??0;
  x.marketState=x.marketState||{eventRisk:x.eventRisk?.riskLevel||'LOW',status:'LIVE'};
  x.position=x.position||{state:'FLAT',qty:0};
  x.reason=x.reason??'NO_OUTCOME_YET';
  x.evidence=x.evidence||{decision:baseDirectionFromContext(base,canonical),price:pxSeed};
  x.trigger=x.trigger??setupSeed?.trigger??'WAIT';
  x.location=x.location??base.liquidity?.rangePosition??.5;
  x.aiOutput=x.aiOutput||{status:'NOT_RUN',verdict:'NO_AI_VERDICT',confidence:0};
  return x;
}
...
function canonicalize(index,result,canonical){
  ...
  switch(index){
    ...
    case 45:Object.assign(canonical,{deterministicAudit:r});break;
    case 46:Object.assign(canonical,{decisionContract:r});break;
...
export function runTitanPipeline(baseInput={}){
  ...
  for(let i=0;i<TITAN_MODULES.length;i++){
    ...
    const input=safeModuleInput(baseInput,canonical,outputs,i+1,module.MODULE.id,moduleTestStatus);
    ...
    const missingContract=Array.isArray(result.diagnostics?.warnings) && result.diagnostics.warnings.some((w)=>String(w).startsWith('MISSING_INPUTS:'));
    const modulePassed=result.diagnostics?.errors?.length===0 && !missingContract;
  ...
  for(const t of traces){
    for(const dep of (DEPENDENCIES[t.id]||[])){
      ...
      else if((depTrace.blockers||[]).length>0&&!AUDIT_ONLY.has(dep))dependencyViolations.push(t.id+"<-"+dep);
    }
  }
  return Object.freeze({
    ...
    summary:{
      ...
      healthyModules:traces.filter(x=>(x.blockers||[]).length===0).length,
      failedModules:traces.filter(x=>(x.blockers||[]).length>0).length,
      businessBlockedModules:traces.filter(x=>x.status==="BLOCKED"&&(x.blockers||[]).length===0).length,
      contractWarnings:traces.filter(x=>(x.blockers||[]).some((b)=>String(b).startsWith("MISSING_INPUTS:"))).length,
