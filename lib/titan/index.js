...
export function runTitanPipeline(baseInput={}){
  ...
  for(const t of traces){
    for(const dep of (DEPENDENCIES[t.id]||[])){
      ...
      else if((depTrace.status==="BLOCKED" || (depTrace.blockers||[]).some((b)=>String(b).startsWith("MISSING_INPUTS:")))&&!AUDIT_ONLY.has(dep))dependencyViolations.push(t.id+"<-"+dep);
    }
  }
  return Object.freeze({
    ...
    summary:{
      ...
      healthyModules:traces.filter(x=>x.status!=="BLOCKED"&&!(x.blockers||[]).some((b)=>String(b).startsWith("MISSING_INPUTS:"))).length,
      blockedModules:traces.filter(x=>x.status==="BLOCKED"||(x.blockers||[]).some((b)=>String(b).startsWith("MISSING_INPUTS:"))).length,
