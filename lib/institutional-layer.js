...
function walkBook(book,side,notional){
  ...
  for(const r of rows){
    ...
    remainingNotional -= Math.min(remainingNotional, r.price*r.size);
    levelsUsed++;
  }
  return {notional,filled,qty:baseQty,complete:remaining<=1e-9,avgPrice:avg,slippageBps:best?Math.abs(avg-best)/best*10000:null,levelsUsed:Math.max(1,levelsUsed)};
}
...
export function buildInstitutionalLayer({
  ...
  const configuredCost=Number(process.env.BACKTEST_COST_BPS);
  const costBps=Number.isFinite(configuredCost)&&configuredCost>0?configuredCost:12;
  const costSource=Number.isFinite(configuredCost)&&configuredCost>0?'ENVIRONMENT_VARIABLE':'CONSERVATIVE_DEFAULT';
  const backtest=baselineBacktest(primary,costBps);
  const walk=walkForward(primary,costBps);
  ...
  return {
    ...
    executionCostModel:{
      ...
      note:'Conservative research assumption; dynamic order-book impact is reported separately and actual account fee tier can be supplied through BACKTEST_COST_BPS.'
    },
    structure:{
    ...
    validation:{
      ...
      executionCostModel:'CONFIGURED_CONSERVATIVE_DEFAULT',
      microstructureHistory:'PERSISTED_MINUTE_SNAPSHOTS_WITH_DEEP_BOOK_AND_RECENT_TRADES',
