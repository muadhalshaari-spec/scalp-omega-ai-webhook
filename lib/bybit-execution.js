  ...
  for(const delay of [250,500,1000]){
    ...
    if(command.action==='OPEN'){
      if(command.orderType==='Limit'){
        ...
        last.verified=Boolean(active||h);
        last.reason=last.verified?'ORDER_VISIBLE':'ORDER_NOT_VISIBLE';
      }else{
...
function riskCheck(command, guard, account){
  const checks={guardLoaded:true,envEnabled:false,killSwitchClear:false,guardEnabled:false,withinMaxLeverage:true,availableBalance:true,positionState:true,notional:true};
  const reasons=[];
