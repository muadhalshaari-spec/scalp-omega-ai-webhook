...
function validateCommand(command){
  ...
  if(command.positionIdx!=null && ![0,1,2].includes(Number(command.positionIdx))) throw new Error('positionIdx must be 0, 1, or 2');
  ...
  if(command.action==='OPEN'){
    ...
    if(orderType==='Limit'){
      assertFinitePositive(command.price,'price');
      if(command.timeInForce && !['GTC','IOC','FOK','PostOnly'].includes(command.timeInForce)) throw new Error('Unsupported timeInForce for Limit order');
    }else{
      assertFinitePositive(command.referencePrice,'referencePrice');
    }
    ...
    if(command.tpTriggerBy && !['LastPrice','IndexPrice','MarkPrice'].includes(command.tpTriggerBy)) throw new Error('Unsupported tpTriggerBy');
    if(command.slTriggerBy && !['LastPrice','IndexPrice','MarkPrice'].includes(command.slTriggerBy)) throw new Error('Unsupported slTriggerBy');
...
async function verifyExecution(command, providerResult, preState, {signal}={}){
  ...
  for(const delay of [250,500,1000]){
    await wait(delay);
    if(signal?.aborted) throw new Error('Execution verification aborted');
    const [account,history] = await Promise.all([
      getBybitPrivateAccount({signal}),
      getBybitOrderHistory({orderId:orderId||undefined,orderLinkId:orderLinkId||undefined,limit:10,signal})
    ]);
...
function riskCheck(command, guard, account){
  const checks={guardLoaded:true,guardConfig:true,envEnabled:false,killSwitchClear:false,guardEnabled:false,withinMaxLeverage:true,availableBalance:true,margin:true,positionState:true,notional:true};
  const maxLeverage=Number(guard.max_leverage);
  const maxEquityPct=Number(guard.max_equity_pct);
  const minAvailable=Number(guard.min_available_usdt);
  const maxNotional=guard.max_notional_usdt==null?null:Number(guard.max_notional_usdt);
  if(!Number.isFinite(maxLeverage)||maxLeverage<1||maxLeverage>100 ||
     !Number.isFinite(maxEquityPct)||maxEquityPct<=0||maxEquityPct>1 ||
     !Number.isFinite(minAvailable)||minAvailable<0 ||
     (maxNotional!=null && (!Number.isFinite(maxNotional)||maxNotional<=0))){
  ...
  if(command.action==='OPEN'){
    ...
    const leverage=Number(command.leverage);
    const equityCap=Number.isFinite(equity)&&equity>0 ? equity*maxEquityPct : NaN;
    const absoluteCap=maxNotional==null ? Infinity : maxNotional;
    const cap=Number.isFinite(equityCap)?Math.min(absoluteCap,equityCap):NaN;
    checks.notional=Number.isFinite(notional)&&notional>0&&Number.isFinite(cap)&&notional<=cap;
    if(!checks.notional) reasons.push('MAX_NOTIONAL');

    const requiredMargin=Number.isFinite(notional)&&Number.isFinite(leverage)&&leverage>0 ? notional/leverage : NaN;
    const available=Number(account?.wallet?.totalAvailableBalance);
    checks.margin=Number.isFinite(requiredMargin)&&Number.isFinite(available)&&requiredMargin<=Math.max(0,available-minAvailable);
    if(!checks.margin) reasons.push('INSUFFICIENT_MARGIN');

    checks.withinMaxLeverage=leverage<=maxLeverage;
    if(!checks.withinMaxLeverage) reasons.push('MAX_LEVERAGE');
  }else if(command.action==='CLOSE'||command.action==='REDUCE'){
...
export async function executeBybitCommand(command,{signal}={}){
  ...
  try{
    ...
    const verification=await verifyExecution(command,providerResult,preState,{signal:controller.signal});
