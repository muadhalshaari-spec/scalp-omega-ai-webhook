...
export async function fetchExternalIntelligence({symbol='ETHUSDT',signal}={}){
  ...
  try{
    ...
    if(deribitSettled.status==='fulfilled'){
      const [t,options]=deribitSettled.value; const optionSummary=summarizeDeribitOptions(options);
      providers.deribit={available:Boolean(t),source:'Deribit',timestamp:num(t?.timestamp),price:num(t?.lastPrice),markPrice:num(t?.markPrice),indexPrice:num(t?.indexPrice),openInterest:num(t?.openInterest),currentFunding:num(t?.currentFunding),funding8h:num(t?.funding8h),iv:num(t?.markIv),options:optionSummary};
    }
