...
export async function fetchExternalIntelligence({symbol='ETHUSDT',signal}={}){
  ...
  try{
    ...
    if(binance.status==='fulfilled' && !Number.isFinite(num(binance.value?.summary?.futuresPrice)) && relayUsable){
      providers.binance={
        available:Number.isFinite(num(relaySummary.binanceMarkPrice)),
        source:'Binance',
        ...
        book:null,
        spotPrice:null,
        futuresSpotBasisPct:null,
        adlRisk:null,
        coverage:{relay:true},
        futures:{premiumIndex:relayBinance?.premiumIndex||null,openInterest:relayBinance?.openInterest||null},
        spot:{},
        apiKeyData:null,
    ...
    if(bybit.status==='fulfilled' && !Number.isFinite(num(bybit.value?.summary?.linearPrice)) && relayUsable){
      providers.bybit={
        ...
        book:null,
        coverage:{relay:true},
        futures:{ticker:relayBybit?.ticker||null,openInterest:relayBybit?.openInterest||null},
        spot:{},
        quality:{live:true,relay:true,directPublicAvailable:false}
      };
    }
