...
import { getRecentMarketCandles } from './supabase.js';
import { getLatestMarketSnapshot } from './supabase.js';


const HISTORY_TIMEFRAMES = ['1m','5m','15m','1H','4H','1D'];

function compactHistoricalRows(rows, limit = 30) {
  const closed = (Array.isArray(rows) ? rows : []).filter(x => x?.confirmed === true).sort((a,b)=>Number(a.time_ms)-Number(b.time_ms));
  const recent = closed.slice(-limit).map(x => [Number(x.time_ms),Number(x.open),Number(x.high),Number(x.low),Number(x.close),Number(x.volume ?? 0)]);
  const anchors = closed.length > limit ? [closed[0], closed[Math.floor(closed.length / 2)]].map(x => [Number(x.time_ms),Number(x.open),Number(x.high),Number(x.low),Number(x.close),Number(x.volume ?? 0)]) : [];
  return { fullHistoryRows: closed.length, oldest: closed[0]?.time_ms ?? null, newest: closed.at(-1)?.time_ms ?? null, representativeBars:[...anchors,...recent] };
}

async function loadExternalHistory(source, instrument) {
  const entries = await Promise.all(HISTORY_TIMEFRAMES.map(async timeframe => {
    try {
      const r = await getRecentMarketCandles({source,instrument,timeframe,limit:1000});
      return [timeframe, compactHistoricalRows(r.rows, 30)];
    } catch {
      return [timeframe, {fullHistoryRows:0,oldest:null,newest:null,representativeBars:[]}];
    }
  }));
...
export async function fetchExternalIntelligence({symbol='ETHUSDT',signal}={}){
  ...
  try{
    const [binance,bybit,deribitSettled,relay]=await Promise.allSettled([
    ...
    const relayUsable=Boolean(relayRow);

    if(binance.status==='fulfilled' && (!Number.isFinite(num(binance.value?.summary?.futuresPrice)) || binance.value?.quality?.publicAudit?.futures?.successful===0)) {
      providers.binance={
        available:relayUsable && Number.isFinite(num(relaySummary.binanceMarkPrice)),
        source:'Binance',
        route:'SUPABASE_MARKET_SYNC_RELAY',
        directAvailable:false,
        timestamp:relayTs||Date.now(),
        price:num(relaySummary.binanceMarkPrice),
        markPrice:num(relaySummary.binanceMarkPrice),
        indexPrice:null,
        fundingRate:num(relaySummary.binanceFundingRate),
        nextFundingTime:null,
        openInterest:num(relayBinance?.openInterest?.data?.[0]?.openInterest),
        book:null,spotPrice:null,futuresSpotBasisPct:null,adlRisk:null,coverage:{relay:true},
        futures:{premiumIndex:relayBinance?.premiumIndex||null,openInterest:relayBinance?.openInterest||null},spot:{},apiKeyData:null,
        quality:{live:true,relay:true,directPublicAvailable:false}
      };
    }

    if(bybit.status==='fulfilled' && !Number.isFinite(num(bybit.value?.summary?.linearPrice)) && relayUsable) {
      providers.bybit={
        available:Number.isFinite(num(relaySummary.bybitPrice)),
        source:'Bybit',
        route:'SUPABASE_MARKET_SYNC_RELAY',
        directAvailable:false,
        timestamp:relayTs||Date.now(),
        environment:'production',
        price:num(relaySummary.bybitPrice),
        markPrice:null,indexPrice:null,
        fundingRate:num(relaySummary.bybitFundingRate),
        nextFundingTime:null,
        openInterest:num(relayBybit?.openInterest?.result?.list?.[0]?.openInterest),
        spotPrice:null,basis:null,basisRate:null,book:null,coverage:{relay:true},futures:{ticker:relayBybit?.ticker||null,openInterest:relayBybit?.openInterest||null},spot:{},quality:{live:true,relay:true,directPublicAvailable:false}
      };
    }
    ...
    return {
      ...
      externalHistory: await Promise.all([loadExternalHistory('BINANCE','ETHUSDT'), loadExternalHistory('BYBIT','ETHUSDT')]).then(([binanceHistory,bybitHistory])=>({BINANCE:binanceHistory,BYBIT:bybitHistory})),
      dataQuality:{
        live:true,providersAvailable:Object.values(providers).filter(p=>p.available).length,
        historicalExternalData:'SUPABASE_PERSISTED',
    ...
    return {
      ...
      externalHistory:{BINANCE:{},BYBIT:{}},
      dataQuality:{live:true,providersAvailable:Object.values(providers).filter(p=>p.available).length,historicalExternalData:'SUPABASE_PERSISTED',partialError:String(error?.message||error||'external intelligence error')}
    };
