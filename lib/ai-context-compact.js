...
function compactExternal(external) {
  ...
  return {
    ...
    news: {
      ...
      items: (Array.isArray(external.news?.items) ? external.news.items : []).slice(0, 20).map(item => ({ source: item.source ?? null, providerType: item.providerType ?? null, title: item.title ?? null, url: item.url ?? null, publishedAt: item.publishedAt ?? null, relevance: item.relevance ?? 0, eventTags: item.eventTags ?? [] }))
    },
    providers,
    persistedHistory
  };
