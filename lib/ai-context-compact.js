const TF_LIMITS = {
  '1m': { recent: 4, anchors: 0 },
  '5m': { recent: 4, anchors: 0 },
  '15m': { recent: 6, anchors: 0 },
  '1H': { recent: 5, anchors: 0 },
  '4H': { recent: 3, anchors: 0 },
  '1D': { recent: 2, anchors: 0 }
};
...
function compactExternal(external) {
  ...
  const compactHistory = Object.fromEntries(
    Object.entries(external.externalHistory || {}).map(([source, byTf]) => [
      source,
      Object.fromEntries(Object.entries(byTf || {}).map(([tf, h]) => [tf, {
        fullHistoryRows: h?.fullHistoryRows ?? 0,
        oldest: h?.oldest ?? null,
        newest: h?.newest ?? null,
        representativeBars: Array.isArray(h?.representativeBars) ? h.representativeBars : []
      }]))
    ])
  );
  const persistedHistory = Object.fromEntries(
    Object.entries(external.persistedHistory || {}).map(([source, byTf]) => [
      source,
      Object.fromEntries(Object.entries(byTf || {}).map(([tf, rows]) => {
        const confirmed = (Array.isArray(rows) ? rows : []).filter(x => x?.confirmed === true);
        const recent = confirmed.slice(-2).map(x => [
          Number(x.time_ms), Number(x.open), Number(x.high), Number(x.low), Number(x.close), Number(x.volume ?? 0)
        ]);
        return [tf, {
          storedRows: confirmed.length,
          oldest: confirmed[0]?.time_ms ?? null,
          newest: confirmed.at(-1)?.time_ms ?? null,
          recentBars: recent
        }];
      }))
    ])
  ...
  return {
    ...
    externalHistory: compactHistory,
    persistedHistory,
...
function compactMarket(market) {
  ...
  const trimBook = (book) => book ? {
    ...
    bids: tail(book.bids, 2),
    asks: tail(book.asks, 2)
  } : null;
  return {
    ...
    oiHistory: tail(market.oiHistory, 1),
    fundingHistory: tail(market.fundingHistory, 1),
    longShortHistory: tail(market.longShortHistory, 1),
    takerVolumeHistory: tail(market.takerVolumeHistory, 1),
    trades: tail(market.trades, 2),
    orderBook: trimBook(market.orderBook),
    liquidations: tail(market.liquidations, 2),
    liquidationHistory: tail(market.liquidationHistory, 2)
  };
}
...
export function compactAiInput(raw) {
  ...
  const compactFeatures = Object.fromEntries(
    Object.entries(raw.featureSummary || {}).map(([tf, f]) => [tf, {
      lastCandle: f?.lastCandle ? {
      ...
      indicators: f?.indicators ? {
        ...
        atr14: f.indicators.atr14 ?? null,
        vwap: f.indicators.vwap ?? null,
        macdHistogram: f.indicators.macdHistogram ?? null,
        volumeRatio20: f.indicators.volumeRatio20 ?? null
      } : null,
  ...
  return {
    ...
    featureSummary: compactFeatures,
