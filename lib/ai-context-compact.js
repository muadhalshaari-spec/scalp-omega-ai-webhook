...
function compactExternal(external) {
  ...
  return {
    ...
    onchain: {
      availableCount: external.onchain?.availableCount ?? Object.values(external.providers || {}).filter(p => p?.source === 'CoinGecko' || p?.source === 'Alchemy' || p?.source === 'Etherscan').filter(p => p.available).length,
      configuredCount: external.onchain?.configuredCount ?? null,
      providers: {
        coingecko: external.providers?.coingecko ? { available: external.providers.coingecko.available === true, ethereum: external.providers.coingecko.ethereum ?? null, bitcoin: external.providers.coingecko.bitcoin ?? null, fetchedAt: external.providers.coingecko.fetchedAt ?? null, error: external.providers.coingecko.error ?? null } : null,
        alchemy: external.providers?.alchemy ? { available: external.providers.alchemy.available === true, network: external.providers.alchemy.network ?? null, latestBlock: external.providers.alchemy.latestBlock ?? null, gasPriceGwei: external.providers.alchemy.gasPriceGwei ?? null, fetchedAt: external.providers.alchemy.fetchedAt ?? null, error: external.providers.alchemy.error ?? null } : null,
        etherscan: external.providers?.etherscan ? { available: external.providers.etherscan.available === true, network: external.providers.etherscan.network ?? null, gasOracle: external.providers.etherscan.gasOracle ?? null, fetchedAt: external.providers.etherscan.fetchedAt ?? null, error: external.providers.etherscan.error ?? null } : null
      }
    },
    news: {
      fetchedAt: external.news?.fetchedAt ?? null,
      itemCount: external.news?.itemCount ?? 0,
      sources: external.news?.sources ?? {},
      sentiment: external.news?.sentiment ?? { analyzedCount: 0, scoredCount: 0, label: 'UNAVAILABLE', method: 'LOCAL_LEXICON_CONTEXT_ONLY' },
      dataQuality: external.news?.dataQuality ?? { newsIsContextOnly: true },
      items: (Array.isArray(external.news?.items) ? external.news.items : []).slice(0, 20).map(item => ({ source: item.source ?? null, providerType: item.providerType ?? null, title: item.title ?? null, url: item.url ?? null, publishedAt: item.publishedAt ?? null, relevance: item.relevance ?? 0, eventTags: item.eventTags ?? [], sentiment: item.sentiment ? { label: item.sentiment.label ?? 'NEUTRAL', polarity: item.sentiment.polarity ?? 0, intensity: item.sentiment.intensity ?? 0, confidence: item.sentiment.confidence ?? 0, polarization: item.sentiment.polarization ?? 0 } : null }))
    },
    providers,
    persistedHistory
  };
...
function compactRealtimeMemory(memory) {
  const wrapped = Boolean(memory && typeof memory === 'object' && Object.prototype.hasOwnProperty.call(memory, 'value'));
  const value = wrapped ? memory.value : memory;
  ...
  return {
    ...
    tradeFlow: value.tradeFlow ?? null,
...
function compactRealtimeStream(stream) {
  if (!stream || typeof stream !== 'object') return { available: false };
  return {
    available: stream.ok !== false && stream.dataReady !== false,
    connected: stream.connected === true,
    dataReady: stream.dataReady === true,
    updatedAt: stream.updatedAt ?? null,
    source: stream.source ?? null,
    instrument: stream.instrument ?? null,
    ticker: stream.ticker ?? null,
    latestTrade: stream.latestTrade ?? null,
    orderBook: stream.orderBook ? { time: stream.orderBook.time ?? null, bids: (stream.orderBook.bids || []).slice(0, 3), asks: (stream.orderBook.asks || []).slice(0, 3) } : null,
    liquidationFeed: stream.liquidationFeed ?? null,
    liquidations: (Array.isArray(stream.liquidations) ? stream.liquidations : []).slice(0, 20),
    candles: Object.fromEntries(Object.entries(stream.candles || {}).map(([tf, row]) => [tf, row ? { time: row.ts ?? row[0] ?? null, open: row.o ?? row[1] ?? null, high: row.h ?? row[2] ?? null, low: row.l ?? row[3] ?? null, close: row.c ?? row[4] ?? null, volume: row.vol ?? row[5] ?? null } : null])),
    missingChannels: stream.missingChannels ?? []
  };
}
...
export function compactAiInput(raw) {
  ...
  return {
    ...
    realtimeStream:compactRealtimeStream(raw.realtimeStream || null),
