const TF_LIMITS = {
  '1m': { recent: 50, anchors: 4 },
  '5m': { recent: 50, anchors: 4 },
  '15m': { recent: 80, anchors: 6 },
  '1H': { recent: 50, anchors: 4 },
  '4H': { recent: 40, anchors: 4 },
  '1D': { recent: 30, anchors: 4 }
};
...
function summarizeCandles(rows, tf) {
  ...
  const cfg = TF_LIMITS[tf] || { recent: 50, anchors: 4 };
    ...
    providers: Object.fromEntries(
      Object.entries(external.providers || {}).map(([name, p]) => {
        ...
        return [name, {
          ...
          options: options ? {
            totalOI: options.totalOI ?? null,
            callOI: options.callOI ?? null,
            putOI: options.putOI ?? null,
            putCallOI: options.putCallOI ?? null,
            weightedIV: options.weightedIV ?? null,
            topExpiries: tail(options.topExpiries, 3)
          } : null
        }];
      })
    )
  ...
  return {
    ...
    series: Object.fromEntries(
      Object.entries(macro.series || {}).map(([id, s]) => [id, {
        id: s.id || id,
        title: s.title || null,
        observations: tail(s.observations, 3)
      }])
    )
  };
}
...
export function compactAiInput(raw, { previousLive = null } = {}) {
  ...
  return {
    ...
    market: {
      ...
      oiHistory: tail(market.oiHistory, 20),
      fundingHistory: tail(market.fundingHistory, 20),
      longShortHistory: tail(market.longShortHistory, 20),
      takerVolumeHistory: tail(market.takerVolumeHistory, 20),
      trades: tail(market.trades, 20),
      orderBook: market.orderBook
        ? {
            time: market.orderBook.time ?? market.orderBook.ts ?? null,
            bids: tail(market.orderBook.bids, 5),
            asks: tail(market.orderBook.asks, 5)
          }
        : null,
      liquidations: tail(market.liquidations, 40),
      liquidationHistory: tail(market.liquidationHistory, 40)
    },
