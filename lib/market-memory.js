...
function compactLive(dataFeed, liveData, previous = null) {
  ...
  const trades = Array.isArray(liveData?.trades) ? liveData.trades : Array.isArray(m.trades) ? m.trades : [];
  const tradeRows = trades.map((trade) => ({
    side: String(trade?.side || '').toLowerCase(),
    price: finite(trade?.px ?? trade?.price),
    size: finite(trade?.sz ?? trade?.size) ?? 0
  })).filter((trade) => trade.size > 0);
  const buySize = tradeRows.filter((trade) => trade.side === 'buy').reduce((sum, trade) => sum + trade.size, 0);
  const sellSize = tradeRows.filter((trade) => trade.side === 'sell').reduce((sum, trade) => sum + trade.size, 0);
  ...
  return {
    ...
    tradeFlow: {
      source: 'RECENT_PUBLIC_TRADES',
      tradeCount: tradeRows.length,
      buySize,
      sellSize,
      deltaSize: buySize - sellSize,
      cvdProxy: buySize - sellSize,
      vwap: totalSize ? tradeRows.reduce((sum, trade) => sum + (trade.price || 0) * trade.size, 0) / totalSize : null,
      observedAt: liveData?.updatedAt || new Date().toISOString()
    },
