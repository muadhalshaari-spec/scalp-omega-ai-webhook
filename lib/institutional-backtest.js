...
function firstAfter(rows,ts){
  ...
  while(lo<hi){
    const mid=(lo+hi)>>1;
    const mt=Number(a[mid]?.ts??a[mid]?.time??a[mid]?.[0]);
    if(Number.isFinite(mt)&&mt>ts)hi=mid;
    else lo=mid+1;
  }
  return lo<a.length?a[lo]:null;
}
...
function derivativeMarket(derivatives,ts){
  ...
  const nextFunding=firstAfter(h.funding,ts);
  return{
    ...
    nextFundingTime:lastFunding?.nextFundingTime??nextFunding?.ts??null,
    oiHistory:oi,
