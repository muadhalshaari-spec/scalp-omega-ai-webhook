...
function asOfClosedWindow(rows, ts, limit = 500) {
  const a = Array.isArray(rows) ? rows : [];
  let lo = 0, hi = a.length;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    const mt = Number(a[mid]?.time ?? a[mid]?.ts ?? a[mid]?.[0]);
    if (Number.isFinite(mt) && mt <= ts) lo = mid + 1;
    else hi = mid;
  }
  const end = lo;
  const start = Math.max(0, end - limit);
  return a.slice(start, end).filter(x => x?.confirmed === true);
}
...
export function runInstitutionalBacktest({
  ...
  const preparedCandles = Object.fromEntries(
    Object.entries(candlesByTf || {}).map(([tf, candles]) => [
      tf,
      (Array.isArray(candles) ? candles : [])
        .filter(x => x && x.confirmed === true)
        .sort((a, b) => Number(a.time ?? a.ts ?? a[0]) - Number(b.time ?? b.ts ?? b[0]))
    ])
  );
  const base=preparedCandles['15m'] || [];
  const rows=[],probRows=[];
  ...
  for(let i=220;i<base.length-horizonBars&&rows.length<maxTrades;i++){
    const history = {};
    for (const [tf, candles] of Object.entries(preparedCandles)) {
      history[tf] = asOfClosedWindow(candles, base[i].time, tf === '15m' ? 500 : 300);
    }
