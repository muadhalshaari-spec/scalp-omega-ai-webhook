...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({ok:true,engine:'SCALP-Ω Market Data Feed v3',decisionAuthority:'TITAN_DETERMINISTIC',decisionPolicy:'NO_DECISION_OUTPUT',source:data.source,instrument:data.instrument,fetchedAt:data.fetchedAt,analysisMode:'DATA_ONLY_CLOSED_CANDLES',market:data.market,features:data.features,contexts:data.contexts,externalIntelligence:data.externalIntelligence,macroContext:data.macroContext||null,observations:data.observations,dataQuality:data.dataQuality,featureSummary:data.featureSummary,memory:{historicalStore:'SUPABASE',realtimeStore:'UPSTASH_REDIS',upstashConfigured:memory.configured,upstashLiveSnapshotCached:memory.value!==null,upstashUpdatedAt:memory.value?.updatedAt||null}});
